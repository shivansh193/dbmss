...
export default function CustomerDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-100 p-8">
      <h1 className="text-3xl font-bold mb-8 text-blue-900">Welcome, Customer!</h1>
      ...
      {suggestions.length > 0 && (
        <div className="mb-4 bg-white border rounded shadow p-2 max-w-xs">
          <div className="font-semibold text-gray-700 mb-1">Suggestions:</div>
          <ul>
            {suggestions.map(s => (
              <li key={s} className="cursor-pointer hover:text-blue-700" onClick={() => { setSearch(s); handleSearch(); }}>{s}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 py-6">
        {products.map((product: any) => (
          <Card
            ...
            className="bg-white rounded-2xl shadow-xl border border-gray-100 hover:shadow-2xl hover:scale-[1.03] transition-all cursor-pointer group overflow-hidden"
          >
            <CardHeader className="p-0" onClick={() => router.push(`/product/${product.id || product.product_id}`)}>
              <div className="relative w-full aspect-[4/3] bg-gradient-to-br from-blue-50 to-purple-100 flex items-center justify-center">
                ...
                <span className="absolute top-3 right-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-3 py-1 rounded-full text-sm font-bold shadow">${product.price}</span>
              </div>
            </CardHeader>
            <CardContent className="p-5">
              <h2 className="text-xl font-extrabold text-blue-900 truncate mb-1">{product.name}</h2>
              <p className="text-gray-500 text-sm mb-2">{product.store?.name || product.store_name} {product.store?.city && `(${product.store.city})`}</p>
              <p className="text-gray-700 text-base mb-4 line-clamp-2 min-h-[2.5em]">{product.description}</p>
              ...
              <button
                className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-2.5 rounded-xl hover:from-blue-700 hover:to-purple-700 font-bold text-base shadow-lg transition mt-2"
                onClick={() => {
                  addToCart({
